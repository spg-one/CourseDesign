...
/* 锁定MECC */
static void s3c2440_nand_mecc_lock(void)
{
...
int nand_read_ll_lp(unsigned char *buf, unsigned long start_addr, int size)
{
	...
	int k, len = NAND_SECTOR_SIZE_LP/4;
	unsigned int mecc[4], rddata;
...
/* Read Page */
		...
		/* Write Address */
//		write_addr_lp(i / NAND_SECTOR_SIZE_LP + 2048 + 40);
		*p = col & 0xff;
		for(j=0; j<10; j++);
		*p = (col>>8) & 0xff;
		for(j=0; j<10; j++);
		...
		*p = (page >> 16) & 0x03;		/* Row Address A28~A29 */
		...
		mecc[0] = (volatile unsigned int )s3c2440nand->NFDATA;
		mecc[1] = (volatile unsigned int )s3c2440nand->NFDATA;
		mecc[2] = (volatile unsigned int )s3c2440nand->NFDATA;
		mecc[3] = (volatile unsigned int )s3c2440nand->NFDATA;
		...
		*p = (col>>8) & 0xff;
		...
		for(k=0; k<4; k++)
		{
			...
			s3c2440_nand_mecc_lock();

			s3c2440nand->NFMECCD0 = ((mecc[k]&0xFF00) << 8) | (mecc[k]&0xFF);
			s3c2440nand->NFMECCD1 = ((mecc[k]&0xFF000000) >> 8) | ((mecc[k]&0xFF0000) >> 16);

			rddata = s3c2440nand->NFESTAT0;
			if((rddata & 0x3) == 0x1)
			{
				buf[(rddata>>7) & 0x7ff] ^= (1 << ((rddata>>4) & 0x7));
			}
			else if((rddata & 0x3) > 0x1)
			{
				while(1);
			}
		}
